            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.REMAX:
                        with _timer("gen_max", timing_raw):
                            ...
                            if not self.async_rollout_mode:
                                ...
                                await self.async_rollout_manager.wake_up()
                                gen_baseline_output = await self.async_rollout_manager.generate_sequences(gen_baseline_batch)
                                await self.async_rollout_manager.sleep()
                            # gen_baseline_output = self.actor_rollout_wg.generate_sequences(gen_baseline_batch)
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        training_scores = reduce_metrics(reward_extra_infos_dict)
                        training_scores = {f"training_scores/{k}": v for k, v in training_scores.items()}
                        metrics.update(training_scores)
