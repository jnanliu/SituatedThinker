...
class AsyncLLMServerManager:
    ...
    def __init__(self, config: DictConfig, worker_group: RayWorkerGroup, *, scheduler_kwargs: Dict[str, Any] = None):
        ...
        server_class = async_server_class(
            ...
            chat=self.config.rollout.async_chat
        )
        ...
        self.scheduler_thread = threading.Thread(target=self._init_scheduler, daemon=True)
        self.scheduler_thread.start()
        self.scheduler_ready.wait()
    ...
    def generate_sequences(self, prompts: DataProto, **sampling_params) -> DataProto:
        ...
        assert self.scheduler is not None, "chat scheduler is not initialized."

        future = asyncio.run_coroutine_threadsafe(
            self.scheduler.generate_sequences(prompts, **sampling_params), self.scheduler_loop)
        return future.result()


def async_server_class(rollout_backend: str, chat: bool) -> Type[AsyncServerBase]:
    ...
    if rollout_backend == "vllm":
        if chat:
            ...
            from verl.workers.rollout.vllm_rollout.vllm_async_server import AsyncvLLMCompletionServer
            return AsyncvLLMCompletionServer
