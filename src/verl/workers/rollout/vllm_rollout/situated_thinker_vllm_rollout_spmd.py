...
from inference_engine.vllm import SituatedThinkerLLM as LLM, SamplingParams
...
class vLLMRollout(BaseRollout):
    ...
    def __init__(self, model_path: str, config: DictConfig, tokenizer, model_hf_config, **kwargs):
        ...
        module_path, var_name = config.interface_zoo.rsplit(".", 1)
        module = importlib.import_module(module_path)
        interface_zoo = getattr(module, var_name)

        self.inference_engine = LLM(
            interface_zoo=interface_zoo,
            model=model_path,
        ...
        setattr(self.inference_engine, "max_model_len", config.max_model_len)
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            outputs = self.inference_engine.generate(
                ...
                use_tqdm=False, # torch.distributed.get_rank() == 0,
                extra_infos_of_prompts=dict_of_list_to_list_of_dict(non_tensor_batch)
            )
            ...
            for output in outputs:
                for sample_id in range(len(output.outputs)):
                    ...
                    response.append(token_ids[:max_length])
            ...
            assert len(response) == len(response_result_mask)
            assert response.shape[-1] == response_result_mask.shape[-1]
            ...
            if self.sampling_params.n > 1 and do_sample:
                ...
                for k in non_tensor_batch:
                    non_tensor_batch[k] = _repeat_interleave(non_tensor_batch[k], self.sampling_params.n)
